...
--[[
    ...
    Original project credits are preserved below.
    Modifications include:
      - Removed startup splash
      - Removed analytics, telemetry, and heartbeat requests
      - Added success/error/warning/info notification helpers
      - Added a load-complete notification
      - Added clearer public-library safety disclosures

    License:
      This modified source remains subject to the Mozilla Public License 2.0
      used by the forked Rayfield source. Keep the repository LICENSE file,
      this notice, and the original attribution when redistributing.

    Privacy:
      BananiUI does not intentionally collect usernames, user IDs, game IDs,
      Discord invites, configuration names, or usage analytics.
      Required UI icon/image assets may still be fetched from the documented
      SiriusSoftwareLtd/Rayfield GitHub asset paths.
]]

if debugX then
	warn('Initialising BananiUI')
end
...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'BananiUI Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local prompt = {
    create = function()
        -- Disabled in BananiUI public build.
    end
}
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("BananiUI | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		...
		if useStudio then
			file = [[
	{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"BananiUI Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"BananiUI Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
]]
		end
			...
			for categoryName, categoryTable in file do
				for settingName, setting in categoryTable do
					...
					if not (settingType == typeof(setting.Value)) then
						warn("BananiUI | Error parsing settings file. '"..settingName.."' must be a "..settingType)
						continue
					end
	...
	if not success then 
		if writefile then
			warn('BananiUI had an issue accessing configuration saving capability.')
		end
	end
end
...
-- BananiUI privacy build:
-- Analytics, telemetry, reporter downloads, and heartbeat requests are disabled.
local reporter = nil
...
local function getCurrentGameName()
    local fallback = tostring(game.Name)
...
repeat
	...
	if not warned then
		warn('BananiUI | Build Mismatch')
		print('BananiUI may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end
		...
		local ok, err = pcall(function()
			...
			for id, _ in assetFiles do
				...
				if success then
					...
					warn("BananiUI | Failed to load custom asset: "..tostring(id).." - "..tostring(asset))
				end
			end
		end)
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("BananiUI | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("BananiUI | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
function BananiUI:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('BananiUI | Not seeing your actions in notifications?')
...
function BananiUI:SafeCall(label, callback, ...)
    ...
    if not success then
        warn("BananiUI | " .. tostring(label) .. " failed:\n" .. tostring(result))

        self:NotifyError(
            tostring(label) .. " failed. Check the developer console for details.",
            "BananiUI Error"
        )
...
function BananiUI:CreateWindow(Settings)
	...
	-- BANANIHUB: prepare the real interface immediately.
	Topbar.Visible = true
	Elements.Visible = true
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("BananiUI | "..Key.." Error " ..tostring(Response))
...
return BananiUI
	...
	-- BANANIHUB: show the main interface with no loading animation.
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("BananiUI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("BananiUI | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("BananiUI | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Rayfield specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("BananiUI | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Rayfield specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("BananiUI | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Rayfield specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("BananiUI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Rayfield specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
	...
	-- BANANIHUB: final visible window state with no splash.
	Elements.Visible = true
	LoadingFrame.Visible = false

	Main.Size = useMobileSizing
		and UDim2.new(0, 500, 0, 275)
		or UDim2.new(0, 500, 0, 475)
	Main.BackgroundTransparency = 0
	Main.Shadow.Image.ImageTransparency = 0.6
	...
	if not success then warn('BananiUI had an issue creating settings.') end
...
if useStudio then
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using BananiUI may overwrite your key file
